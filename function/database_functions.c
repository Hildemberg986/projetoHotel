...
Reservation *search_reservation_cpf(const char *cpf)
{
    ...
    if (arquivo != NULL)
    {
        ...
        while (fread(loaded_reservation, sizeof(Reservation), 1, arquivo) == 1)
        {
            ...
            {
                printf("ID da Reserva: %s",loaded_reservation->reservation_number);
                printf("ID do Quarto: %s",loaded_reservation->room_number);
                printf("Dia de entrada: %s | Dia de Saída: %s\n", loaded_reservation->day_enter,loaded_reservation->day_exit);
                printf("\n");   
            }
        }
        // Fecha o arquivo aberto
        fclose(arquivo);
        printf("Não a Reservas Para Esse CPF.\n");
        return NULL;
    }
    else
    {
